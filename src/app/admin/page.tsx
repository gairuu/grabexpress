    ...
    <div className="min-h-screen bg-[#f3f5f7] text-[#1f2937] flex flex-col">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-6 md:p-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
          ...
          <StatsCard icon={Wallet} label="Revenue" value={formatCurrency(deliveries.reduce((a, b) => a + b.delivery_fee, 0))} color="#00D861" delay={150} />
        </div>
              ...
              <tbody className="divide-y divide-[#e5e7eb]">
                {activeTab === 'deliveries' && deliveries.map((d) => (
                  <tr key={d.id} className="hover:bg-[#f9fafb] transition-colors">
                    ...
                    <td className="p-5 text-sm text-[#111827] font-medium">{d.customer_name}</td>
                    <td className="p-5 text-sm text-[#6b7280]">{d.driver_name}</td>
                    <td className="p-5 text-sm text-[#9ca3af] max-w-xs truncate">{d.pickup_location} → {d.dropoff_location}</td>
                    <td className="p-5"><StatusBadge status={d.delivery_status} size="sm" /></td>
                    <td className="p-5 text-sm font-bold text-[var(--grab-green)] text-right">{formatCurrency(d.delivery_fee)}</td>
                  </tr>
                ))}
                ...
                {activeTab === 'customers' && customers.map((c) => (
                  <tr key={c.id} className="hover:bg-[#f9fafb] transition-colors">
                    ...
                    <td className="p-5 text-sm text-[#6b7280]">{c.contact_number || '—'}</td>
                ...
                {activeTab === 'drivers' && drivers.map((d) => (
                  <tr key={d.id} className="hover:bg-[#f9fafb] transition-colors">
                    ...
                    <td className="p-5 text-right">
                      <span className={`text-[10px] uppercase font-bold px-2 py-1 rounded-full ${d.status === 'available' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
